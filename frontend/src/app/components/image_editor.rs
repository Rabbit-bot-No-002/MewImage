...
fn LoadEditor(thread_id: String) -> impl IntoView {
    ...
    view! {
        <div class="image-editor-backdrop">
            {move || loaded.get().map(|draft| view! { <EditorDialog initial=draft /> })}
            <Show when=move || loaded.with(Option::is_none)>
                <section node_ref=loading_dialog tabindex="-1" class="image-editor-dialog stack" role="dialog" aria-modal="true" aria-label="加载编辑草稿"
...
fn prepare_editor_draft(
    ...
    let Some(base_id) = requested_base else {
        return previous.map(|draft| Ok((draft, false))).unwrap_or_else(|| {
            EditorDraft::new(thread_id.into(), None, dimensions.0, dimensions.1)
                .map(|draft| (draft, false))
        });
    };
...
mod tests {
    ...
    fn changing_base_requires_confirmation_and_does_not_reuse_old_layers() {
        ...
        let (restored, confirm) =
            prepare_editor_draft("thread", Some("old"), Some(previous.clone()), &[], (64, 64))
                .unwrap();
        assert!(!confirm);
        assert_eq!(restored, previous);
        assert!(prepare_editor_draft("thread", Some("missing"), None, &[asset], (64, 64)).is_err());
    }
}

#[component]
fn EditorDialog(initial: EditorDraft) -> impl IntoView {
    let ui = expect_context::<UiState>();
    let workspace = expect_context::<WorkspaceState>();
    let composer = expect_context::<ComposerState>();
    ...
    let saved_revision = RwSignal::new(None::<u64>);
        ...
        <section node_ref=dialog tabindex="-1" class="image-editor-dialog stack" role="dialog" aria-modal="true" aria-label="图像编辑器"
            on:keydown=move |event: web_sys::KeyboardEvent| {
                if event.key() == "Escape" {
                    ...
                    else if confirm_clear.get_untracked() { confirm_clear.set(false); } else { save(true); }
                }
            }>
            <header class="row"><h2>"图像编辑器"</h2><button class="button ghost" disabled=applying on:click=move |_| save(true)>"保存并关闭"</button></header>
            <fieldset disabled=busy class="image-editor-controls row">
                {[(crate::image_editor::EditMode::Mask, "局部修改"), (crate::image_editor::EditMode::Annotation, "标记"), (crate::image_editor::EditMode::Sketch, "草图")].into_iter().map(|(mode, label)| view! {
                    <button class="button ghost" aria-pressed=move || draft.with(|draft| draft.mode == mode)
                        ...
                        on:click=move |_| {
                            ...
                            tool.set(DrawTool::Pen); erase.set(false); selected.set(None); text_edit.set(None); refresh();
                        }>{label}</button>
                }).collect_view()}
                <button class="button ghost" disabled=drawing on:click=move |_| reset_view.update(|value| *value = value.wrapping_add(1))>"适应窗口"</button>
                {[DrawTool::Pen, DrawTool::Arrow, DrawTool::Rectangle, DrawTool::Ellipse, DrawTool::Text, DrawTool::Select].into_iter().map(|choice| view! {
                    <button class="button ghost" disabled=move || drawing.get() || (choice != DrawTool::Pen && draft.with(|draft| draft.mode == crate::image_editor::EditMode::Mask)) aria-pressed=move || tool.get() == choice && !erase.get()
                        on:click=move |_| { tool.set(choice); erase.set(false); }>{choice.label()}</button>
                }).collect_view()}
                <button class="button ghost" disabled=drawing aria-pressed=erase on:click=move |_| { erase.set(true); tool.set(DrawTool::Pen); }>"橡皮"</button>
                ...
                <label>"笔刷 "<input type="range" min="1" max="128" prop:value=brush_width on:input=move |event| {
            ...
            <Show when=move || draft.with(|draft| draft.mode == crate::image_editor::EditMode::Mask)>
                <div class="row">
                    <p class="muted">"蓝色区域为待修改选区。遮罩仅引导修改，模型可能影响选区之外的内容。"</p>
                    <label class="button ghost">"导入同尺寸 Alpha PNG"
            ...
            <Show when=move || selected.with(Option::is_some)>
                <div class="row">
                    <span>"已选中对象：拖动可移动"</span>
                    <button class="button ghost" disabled=busy on:click=move |_| {
                    ...
                    <button class="button ghost danger" disabled=busy on:click=move |_| {
                        if let Some(id) = selected.get_untracked() {
            ...
            <text::TextEditor pending=text_edit on_save=Callback::new(move |edit: text::TextEdit| {
                ...
                session.update_value(|session| {
                    result = if let Some(id) = edit.id { session.edit_text(&id, edit.value) }
                        ...
                        color: color.get_untracked(), width: brush_width.get_untracked().max(12.0),
                    }) };
                });
