...
- 连续修改已升级为带轮次和分支的图像对话：Responses API 在服务商配置未变化时使用 `previous_response_id` 延续上游会话，Images API 与旧中转站使用本地整理的初始要求和最近 20 轮历史。工作台时间线可查看、编辑或从任一结果创建分支；编辑当前连续底图不会改变普通参考图顺序。
- 后端以 `image_conversation_v1` 声明连续会话能力；旧后端会使用预先生成的兼容上下文。原生 Response ID 失效时不会自动重发，可在时间线标记“重建上下文”后手动重试。
...
- 同一站点与协议的服务商配置可保存多个图片模型；设置页负责增删及选择模型，工作台按服务商分组一键切换。地址、API Key、协议和输出设置由组内模型共享，Responses 主模型仍独立配置。
...
旧服务商配置升级后只保留原来正在使用的模型，不会自动改写模型列表；新建 OpenAI Image 配置会预置 `gpt-image-2`、`gpt-image-2.5-flare` 和 `gpt-image-2.5-sunburst`。模型切换会随配置保存并参与手动云同步。包含多个模型的配置要求后端健康能力 `provider_model_lists_v1`，旧后端会在同步前被明确阻止，但仍可使用当前模型完成代理生成。

“液态玻璃”使用低模糊、高透明面板和折射光纹，让自定义背景在卡片后方保持可辨认。大型面板只使用约 5px 的轻度模糊，重复画廊卡片不单独模糊；不支持 `backdrop-filter` 的浏览器会自动使用更高不透明度的安全回退样式。

## 模板广场

管理员点击“导出模板”可选择“分类分享”或“完整备份”。分类分享支持多选整个分类或具体标签，命中任意一项即可；默认仅导出已发布模板，可额外勾选草稿和归档。“未分类”包含普通标签及没有标签的模板。导出窗口会显示匹配模板数、去重后的原图数量和大小；每个模板保留全部标签、参数、预览图和参考图，分类包不受页面分页限制。完整备份包含全部状态的模板。

模板 ZIP 可由不同网站的管理员直接互相分享，不绑定账号或域名，也不依赖原站图片链接。接收方使用自己的生图服务商配置，导入仍受接收站点的图片大小、广场容量和文件数量限制。ZIP 格式保持 Schema v1，旧模板包继续可用。

合并导入默认“保留本地版本”：同 UUID 模板跳过，新模板正常导入；选择“使用包内版本”则覆盖同 UUID 模板，包外模板及现有点赞保留。共享后分别编辑的模板仍具有相同 UUID，请按需要选择策略。图片 ID 冲突会安全重映射，避免覆盖其他模板的图片。全量替换仍会清空整个广场及点赞，即使选择的是单一分类包也一样。

本次新增接口需要前后端一起更新：本地开发修改后端后应重新执行 `cargo run -p mew-image-backend`；仅由 Trunk 热更新前端不会更新后台进程。旧后端不支持冲突策略时，新前端会阻止合并导入并提示更新。旧客户端不传冲突策略时仍按原有覆盖规则处理。
