...
- 代理生图使用后台任务和短轮询，耗时数分钟的生成不会持续占用同一条反向代理连接
...
队列模式最多保留 20 个活动任务卡，浏览器和后端会根据参考图字节数、分辨率、输出数量及可用内存动态控制真正执行的并发数。等待卡可单独停止，设置栏也可停止全部；确认取消后卡片会直接移除，但已经到达上游的请求仍可能产生费用。第 21 个活动任务会被明确拒绝，不会继续占用内存。
...
页面左上角可在“工作台 / 模板广场”间切换，收藏按钮仍保留在原位置，Logo 与标题保持居中。模板广场默认显示最新发布内容，每页 24 项；搜索覆盖标题、提示词和标签，多标签筛选要求模板同时具备全部所选标签。标签筛选采用“分类 → 标签”的两级弹出菜单，管理员可在编辑模板时使用“`分类/标签`”路径归类，历史普通标签会自动显示在“未分类”。分享链接使用 `/?view=templates&template={模板UUID}`，无需额外的前端路由配置。
