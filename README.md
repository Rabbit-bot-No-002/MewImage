...
页面左上角可在“工作台 / 模板广场”间切换，收藏按钮仍保留在原位置，Logo 与标题保持居中。模板广场默认显示最新发布内容，每页 24 项；搜索覆盖标题、提示词和标签，多标签筛选要求模板同时具备全部所选标签。分享链接使用 `/?view=templates&template={模板UUID}`，无需额外的前端路由配置。
